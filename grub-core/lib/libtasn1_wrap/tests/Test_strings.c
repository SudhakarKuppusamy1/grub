...
static const struct tv tv[] = {
  ...
  { ASN1_ETYPE_PRINTABLE_STRING, 5, "\x4e\x69\x6b\x6f\x73", 7, "\x13\x05\x4e\x69\x6b\x6f\x73" },
  { ASN1_ETYPE_UTF8_STRING, 12, "Αττική", 14,
  ...
  { ASN1_ETYPE_OCTET_STRING, 36,
    "\x30\x22\x80\x0F\x32\x30\x31\x31\x30\x38\x32\x31\x30\x38\x30\x30\x30\x36"
    "\x5A\x81\x0F\x32\x30\x31\x31\x30\x38\x32\x33\x32\x30\x35\x39\x35\x39\x5A",
...
static const struct tv ber[] = {
  { ASN1_ETYPE_OCTET_STRING, SSTR ("\xa0\xa0"), SSTR ("\x24\x80\x04\x82\x00\x02\xa0\xa0\x00\x00") },
  { ASN1_ETYPE_OCTET_STRING, SSTR ("\xa0\xa0\xb0\xb0\xb0"),
    SSTR ("\x24\x80\x04\x82\x00\x02\xa0\xa0\x04\x82\x00\x03\xb0\xb0\xb0\x00"
          "\x00") },
  { ASN1_ETYPE_OCTET_STRING, SSTR ("\xa0\xa0\xb0\xb0\xb0\xa1\xa1"),
    SSTR ("\x24\x80\x04\x82\x00\x02\xa0\xa0\x04\x82\x00\x03\xb0\xb0\xb0\x24\x80"
          "\x04\x82\x00\x02\xa1\xa1\x00\x00\x00\x00") },
  { ASN1_ETYPE_OCTET_STRING, SSTR ("\xa0\xa0\xb0\xb0\xb0\xa1\xa1\xc1"),
    SSTR ("\x24\x80\x04\x82\x00\x02\xa0\xa0\x04\x82\x00\x03\xb0\xb0\xb0\x24\x80"
          "\x04\x82\x00\x02\xa1\xa1\x04\x82\x00\x01\xc1\x00\x00\x00\x00") },
};
...
test_strings ()
{
  ...
  for (i = 0; i < sizeof (tv) / sizeof (tv[0]); i++)
    {
      ...
      if (der_len != tv[i].der_len || grub_memcmp (tl, tv[i].der, tl_len) != 0)
        {
          grub_fatal ("DER encoding differs in %u! (size: %u, expected: %u)\n",
      ...
      if (str_len != tv[i].str_len || grub_memcmp (str, tv[i].str, str_len) != 0)
        {
          grub_fatal ("DER decoded data differ in %u! (size: %u, expected: %u)\n",
  ...
  for (i = 0; i < sizeof (ber) / sizeof (ber[0]); i++)
    {
      /* decoding */
      ret = asn1_decode_simple_ber (ber[i].etype, ber[i].der, ber[i].der_len,
                                    &b, &str_len, NULL);
      if (ret != ASN1_SUCCESS)
        {
      ...
      if (str_len != ber[i].str_len || grub_memcmp (b, ber[i].str, str_len) != 0)
        {
          grub_fatal ("BER decoded data differ in %u! (size: %u, expected: %u)\n",
