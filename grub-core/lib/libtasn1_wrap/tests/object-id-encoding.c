  ...
  for (i = 0; i < (int) (sizeof (tv) / sizeof (tv[0])); i++)
    {
      ...
      if (ret != ASN1_SUCCESS)
        {
          if (ret == tv[i].expected_error)
            continue;
          grub_fatal ("%d: iter %lu, encoding of OID failed: %s\n", __LINE__,
                      (unsigned long) i, asn1_strerror (ret));
          return;
        }
      else if (ret != tv[i].expected_error)
        {
          grub_fatal ("%d: iter %lu, encoding of OID %s succeeded when "
                      "expecting failure\n",
      ...
      if (der_len != tv[i].der_len || grub_memcmp (der, tv[i].der, der_len) != 0)
        {
          grub_fatal ("%d: iter %lu, re-encoding of OID %s resulted to "
