 ...
 */
int
_asn1_convert_integer (const unsigned char *value, unsigned char *value_out, int value_out_size, int *len)
{
  char negative;
...
_asn1_append_sequence_set (asn1_node node, struct node_tail_cache_st *pcache)
{
  ...
  p = node->down;
  while ((type_field (p->type) == ASN1_ETYPE_TAG) || (type_field (p->type) == ASN1_ETYPE_SIZE))
    p = p->right;
  ...
  switch (type)
    {
      ...
      case ASN1_ETYPE_ENUMERATED:
                        ...
                        if (!_asn1_strcmp (p->name, value))
                          {
                            ...
                            _asn1_convert_integer (p->value, value_temp,
        ...
        if ((negative && !(value_temp[k] & 0x80)) || (!negative && (value_temp[k] & 0x80)))
          k--;
                        ...
                        if (!_asn1_strcmp (p2->name, p->value))
                          {
                            ...
                            _asn1_convert_integer (p2->value, default_temp,
                                                   SIZEOF_UNSIGNED_LONG_INT, &len2);
                            break;
                          }
                      }
              ...
              case 17:
                ...
                for (k = 13; k < 17; k++)
                  if (!c_isdigit (value[k]))
                    return ASN1_VALUE_NOT_VALID;
                break;
              default: return ASN1_VALUE_NOT_FOUND;
            }
      ...
      case ASN1_ETYPE_CHOICE:
        ...
        if (!p)
          return ASN1_ELEMENT_NOT_FOUND;
        break;
      case ASN1_ETYPE_ANY: _asn1_set_value_lv (node, value, len); break;
      ...
      case ASN1_ETYPE_SET_OF:
        ...
        break;
      default: return ASN1_ELEMENT_NOT_FOUND; break;
    }
...
asn1_read_value_type (asn1_node_const root, const char *name, void *ivalue, int *len, unsigned int *etype)
{
  ...
  if ((type != ASN1_ETYPE_NULL) && (type != ASN1_ETYPE_CHOICE) &&
      !(node->type & CONST_DEFAULT) && !(node->type & CONST_ASSIGN) && (node->value == NULL))
  ...
  switch (type)
    {
      case ASN1_ETYPE_NULL: PUT_STR_VALUE (value, value_size, "NULL"); break;
      case ASN1_ETYPE_BOOLEAN:
        ...
        if ((node->type & CONST_DEFAULT) && (node->value == NULL))
          {
            ...
            while (type_field (p->type) != ASN1_ETYPE_DEFAULT)
              p = p->right;
            if ((c_isdigit (p->value[0])) || (p->value[0] == '-') || (p->value[0] == '+'))
              {
                result = _asn1_convert_integer (p->value, value, value_size, len);
                    ...
                    if (type_field (p2->type) == ASN1_ETYPE_CONSTANT)
                      {
                        if (!_asn1_strcmp (p2->name, p->value))
                          {
                            result = _asn1_convert_integer (p2->value, value,
                                                            value_size, len);
                            if (result != ASN1_SUCCESS)
                              return result;
                            break;
          ...
          {
            len2 = -1;
            result = asn1_get_octet_der (node->value, node->value_len, &len2,
                                         value, value_size, len);
            if (result != ASN1_SUCCESS)
              return result;
          }
        break;
      ...
      case ASN1_ETYPE_VISIBLE_STRING:
        len2 = -1;
        result = asn1_get_octet_der (node->value, node->value_len, &len2, value,
                                     value_size, len);
        if (result != ASN1_SUCCESS)
          return result;
        break;
      case ASN1_ETYPE_BIT_STRING:
        len2 = -1;
        result = asn1_get_bit_der (node->value, node->value_len, &len2, value,
                                   value_size, len);
        if (result != ASN1_SUCCESS)
          return result;
        break;
      ...
      switch (type)
        ...
        CASE_HANDLED_ETYPES:
          *tagValue = _asn1_tags[type].tag;
          ...
          case ASN1_ETYPE_TAG:
          case ASN1_ETYPE_CHOICE:
          case ASN1_ETYPE_ANY: *tagValue = -1; break;
          default: break;
        }
    }
