...
#endif
__attribute__((__pure__)) static unsigned int
  ...
  while (!((p == node) && (move == UP)))
    {
      if (move != UP)
        {
          if ((type_field (p->type) == ASN1_ETYPE_OBJECT_ID) && (p->type & CONST_DEFAULT))
            {
              p2 = p->down;
                      ...
                      if (type_field (p4->type) == ASN1_ETYPE_CONSTANT)
                        {
                          ...
                          if (name2[0])
                            _asn1_str_cat (name2, sizeof (name2), ".");
                          _asn1_str_cat (name2, sizeof (name2), (char *) p4->value);
                        }
                      p4 = p4->right;
                    }
                  tlen = strlen (name2);
  ...
  while (p)
    {
      ...
      else if ((type_field (p->type) == ASN1_ETYPE_OBJECT_ID) && (p->type & CONST_DEFAULT))
        {
          p2 = p->down;
          if (p2 && (type_field (p2->type) == ASN1_ETYPE_DEFAULT))
            {
      ...
      else if ((type_field (p->type) == ASN1_ETYPE_OBJECT_ID) && (p->type & CONST_ASSIGN))
        {
          p2 = p->down;
          if (p2 && (type_field (p2->type) == ASN1_ETYPE_CONSTANT))
            {
