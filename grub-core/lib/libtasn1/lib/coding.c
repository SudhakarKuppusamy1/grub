 ...
 */
static void
_asn1_error_description_value_not_found (asn1_node node, char *ErrorDescription)
{
...
static const unsigned char bit_mask[] = { 0xFF, 0xFE, 0xFC, 0xF8, 0xF0, 0xE0, 0xC0, 0x80 };
 ...
 */
static int
_asn1_complete_explicit_tag (asn1_node node, unsigned char *der, int *counter, int *max_len)
{
  asn1_node p;
 ...
 */
static int
_asn1_insert_tag_der (asn1_node node, unsigned char *der, int *counter, int *max_len)
{
  asn1_node p;
              ...
              if (p->type & CONST_EXPLICIT)
                {
                  if (is_tag_implicit)
                    ...
                    _asn1_tag_der (class | ASN1_CLASS_STRUCTURED,
                                   _asn1_strtoul (p->value, NULL, 10), tag_der, &tag_len);
                  *max_len -= tag_len;
                  if (der && *max_len >= 0)
  ...
  if (is_tag_implicit)
  ...
  else
    {
      ...
      switch (type)
        {
          CASE_HANDLED_ETYPES:
            _asn1_tag_der (_asn1_tags[type].class, _asn1_tags[type].tag, tag_der, &tag_len);
            break;
          case ASN1_ETYPE_TAG:
  ...
  while (p)
    {
      ...
      err = asn1_get_tag_der (der + counter, der_len - counter, &class, &len2, &tag);
      if (err != ASN1_SUCCESS)
        goto error;
  ...
  while (p)
    {
      ...
      if (der_len - counter > 0)
        {
          err = asn1_get_tag_der (der + counter, der_len - counter, &class, &len, NULL);
          if (err != ASN1_SUCCESS)
            goto error;
      ...
      switch (type_field (p->type))
        {
          ...
          case ASN1_ETYPE_OBJECT_ID:
            if ((p->type & CONST_DEFAULT) && (p->value == NULL))
            ...
            else
              {
                ...
                len2 = max_len;
                err = _asn1_object_id_der ((char *) p->value, der ? der +
                                           counter : &dummy, &len2);
                if (err != ASN1_SUCCESS && err != ASN1_MEM_ERROR)
                  goto error;
          ...
          case ASN1_ETYPE_UTC_TIME:
            ...
            len2 = max_len;
            err = _asn1_time_der (p->value, p->value_len, der ? der +
                                  counter : &dummy, &len2);
           if (err != ASN1_SUCCESS && err != ASN1_MEM_ERROR)
             goto error;
