...
#define IS_ERR(len, flags) \
  (len < -1 || ((flags & ASN1_DECODE_FLAG_STRICT_DER) && len < 0))
 ...
 * (useful when no tags are present). DECODE_FLAG_LEVEL1: Internal flag to
 * indicate a level of recursion for BER strings. DECODE_FLAG_LEVEL2: Internal
 * flag to indicate two levels of recursion for BER strings. DECODE_FLAG_LEVEL3:
 * Internal flag to indicate three levels of recursion for BER strings. This is
 * the maximum levels of recursion possible to prevent stack exhaustion.
 */
...
static int
_asn1_decode_simple_ber (unsigned int etype, const unsigned char *der, unsigned int _der_len,
                         unsigned char **str, unsigned int *str_len, unsigned int *ber_len,
                         unsigned dflags);

static int
_asn1_decode_simple_der (unsigned int etype, const unsigned char *der, unsigned int _der_len,
                         const unsigned char **str, unsigned int *str_len, unsigned dflags);
 ...
 */
int
asn1_get_tag_der (const unsigned char *der, int der_len, unsigned char *cls,
 ...
 */
int
asn1_get_octet_der (const unsigned char *der, int der_len, int *ret_len,
 ...
 *
 * Performs basic checks in the DER encoded time object and returns its textual
 * form. The textual form will be in the YYYYMMDD000000Z format for
 * GeneralizedTime and YYMMDD000000Z for UTCTime.
 *
 ...
 */
int
asn1_get_bit_der (const unsigned char *der, int der_len, int *ret_len,
                  unsigned char *str, int str_size, int *bit_len)
{
          ...
          if (type_field (p->type) == ASN1_ETYPE_TAG)
            {
              ...
              if (p->type & CONST_EXPLICIT)
                {
                  if (asn1_get_tag_der (der + counter, der_len, &class, &len2, &tag) != ASN1_SUCCESS)
                    return ASN1_DER_ERROR;
                  ...
                  if (flags & ASN1_DECODE_FLAG_STRICT_DER)
                    len3 = asn1_get_length_der (der + counter, der_len, &len2);
                  else
                    len3 = asn1_get_length_ber (der + counter, der_len, &len2);
      ...
      switch (type)
        {
          ...
          case ASN1_ETYPE_OCTET_STRING:
            ...
            if (((class != ASN1_CLASS_UNIVERSAL) &&
              ...
              return ASN1_DER_ERROR;
            break;
          case ASN1_ETYPE_ANY: counter -= len2; break;
          case ASN1_ETYPE_CHOICE: counter -= len2; break;
          default: return ASN1_DER_ERROR; break;
        }
    }
                    ...
                    unsigned int flags, char *errorDescription)
{
      ...
      if (move != UP)
        {
              ...
              while (p2)
                {
                  if ((p2->type & CONST_SET) && (p2->type & CONST_NOT_USED))
                    {
                      ris = extract_tag_der_recursive (p2, der + counter, ider_len,
                                                       &len2, NULL, flags);
                      if (ris == ASN1_SUCCESS)
                        {
          ...
          if (type_field (p->type) == ASN1_ETYPE_CHOICE)
            {
              while (p->down)
                {
                  ris = extract_tag_der_recursive (p->down, der + counter,
          ...
          if (ris == ASN1_SUCCESS)
            ris = extract_tag_der_recursive (p, der + counter, ider_len,
          ...
          switch (type_field (p->type))
            {
              ...
              case ASN1_ETYPE_ENUMERATED:
                ...
                break;
              case ASN1_ETYPE_OBJECT_ID:
                result = asn1_get_object_id_der (der + counter, ider_len, &len2,
                                                 temp, sizeof (temp));
              ...
              case ASN1_ETYPE_GENERALIZED_TIME:
              case ASN1_ETYPE_UTC_TIME:
                result = _asn1_get_time_der (type_field (p->type), der + counter, ider_len,
                                             &len2, temp, sizeof (temp) - 1, flags);
                if (result != ASN1_SUCCESS)
                  {
                    warn ();
                    goto cleanup;
              ...
              case ASN1_ETYPE_ANY:
                ...
                if (asn1_get_tag_der (der + counter, ider_len, &class, &len2, &tag) != ASN1_SUCCESS)
                  {
                    result = ASN1_DER_ERROR;
                    warn ();
                    goto cleanup;
                ...
                len4 = asn1_get_length_der (der + counter + len2, ider_len, &len3);
                ...
                if (len4 != -1) /* definite */
                  ...
                  {                   /* indefinite length */
                    ...
                    result = _asn1_get_indefinite_length_string (der + counter,
                                                                 ider_len, &len2);
                    if (result != ASN1_SUCCESS)
                      {
                        warn ();
                 ...
                 * with an indefinite length method.
                 */
                if (indefinite)
                  {
                    DECR_LEN (ider_len, 2);
                ...
                move = RIGHT;
                break;
              default: move = (move == UP) ? RIGHT : DOWN; break;
            }
        }
  ...
  if ((ider_len < 0) || (!(flags & ASN1_DECODE_FLAG_ALLOW_PADDING) && (ider_len != 0)))
    {
      warn ();
      result = ASN1_DER_ERROR;
      goto cleanup;
                ...
                while (p2)
                  {
                    if ((type_field (p2->type) == ASN1_ETYPE_OBJECT_ID) &&
                        (p2->type & CONST_ASSIGN))
                      {
                        ...
                        if ((result == ASN1_SUCCESS) && (!_asn1_strcmp (p3->value, value)))
                          {
                                ...
                                if (result == ASN1_SUCCESS)
                                  {
                                    ...
                                    result = asn1_der_decoding (&aux, p->value + len3,
                          ...
                          const char *octetName, const char *objectName)
{
  ...
  while (p2)
    {
      if ((type_field (p2->type) == ASN1_ETYPE_OBJECT_ID) && (p2->type & CONST_ASSIGN))
        {
          ...
          if ((result == ASN1_SUCCESS) && (!_asn1_strcmp (objectNode->value, value)))
            {

              p2 = p2->right; /* pointer to the structure to use for expansion */
              while ((p2) && (p2->type & CONST_ASSIGN))
                p2 = p2->right;
                  ...
                  if (result == ASN1_SUCCESS)
                    {
                      ...
                      len2 = asn1_get_length_der (octetNode->value,
                                                  octetNode->value_len, &len3);
                      if (len2 < 0)
                        return ASN1_DER_ERROR;

                      result = asn1_der_decoding (&aux, octetNode->value + len3,
                                                  len2, errorDescription);
                      if (result == ASN1_SUCCESS)
                        {
...
static int
append (uint8_t **dst, unsigned *dst_size, const unsigned char *src, unsigned src_size)
{
  if (src_size == 0)
    return ASN1_SUCCESS;
