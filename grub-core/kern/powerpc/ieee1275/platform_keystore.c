...
/*
 * GCC >= 9 gives an error taking address of packed member may result in an
 * unaligned pointer value for -Waddress-of-packed-member.
 * A grub_pks_t used packed members, so it needed "-Waddress-of-packed-member" in this file.
 */
#if __GNUC__ >= 9
...
grub_pks_keystore_init (void)
{
  ...
  if (rc == GRUB_ERR_NONE)
    {
      /* Read dbx from PKS. */
      rc = read_sbvar_from_pks (0, DBX, &pks_keystore.dbx, &pks_keystore.dbx_entries);
