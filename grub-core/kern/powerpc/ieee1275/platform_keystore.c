                     ...
                     grub_uint8_t **out, grub_size_t *outlen)
{
  ...
  *out = grub_malloc (pks_max_object_size);
  ...
  rc = grub_ieee1275_pks_read_sbvar (sbvarflags, sbvartype, pks_max_object_size, *out, outlen);
  if (rc == IEEE1275_CELL_NOT_FOUND)
    return grub_error (GRUB_ERR_UNKNOWN_COMMAND, "secure boot variable %s not found (%d)",
                       (sbvartype == DB ? "db" : "dbx"), rc);
  else if (rc < 0)
    return grub_error (GRUB_ERR_READ_ERROR, "secure boot variable %s reading (%d)",
                       (sbvartype == DB ? "db" : "dbx"), rc);
...
is_support_pks (void)
{
  ...
  if (rc < 0 || missing == IEEE1275_CELL_INVALID)
    {
      grub_dprintf ("ieee1275", "warning: firmware doesn't have PKS support\n");
      return GRUB_ERR_BAD_FIRMWARE;
    }
  else
    {
      ...
      if (rc < 0)
        {
          grub_dprintf ("ieee1275", "warning: PKS support is there but it has zero objects\n");
...
/* Load the Platform KeyStore. */
grub_err_t
grub_pks_load_keystore (void)
{
  grub_err_t rc = GRUB_ERR_READ_ERROR;
