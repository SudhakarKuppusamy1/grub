...
process_content (grub_uint8_t *content, int size, struct pkcs7_signedData *msg)
{
  ...
  if (res != ASN1_SUCCESS)
    {
      err = grub_error (GRUB_ERR_BAD_SIGNATURE,
                        N_("error reading PKCS#7 signed data: %s"), asn1_error);
  ...
  if (res != ASN1_SUCCESS)
    {
      err = grub_error (GRUB_ERR_BAD_SIGNATURE, N_("error reading signedData version: %s"),
                        asn1_strerror (res));
      goto cleanup_signed_part;
    }
  ...
  if (version != 1)
    {
      err = grub_error (GRUB_ERR_BAD_SIGNATURE,
                        N_("unexpected signature version v%d, only v1 supported"), version);
      goto cleanup_signed_part;
    }
  ...
  if (res != ASN1_SUCCESS)
    {
      err = grub_error (GRUB_ERR_BAD_SIGNATURE, N_("error counting number of digest algorithms: %s"),
                        asn1_strerror (res));
      goto cleanup_signed_part;
    }
  ...
  for (i = 0; i < algo_count; i++)
    {
      ...
      if (res != ASN1_SUCCESS)
        {
          err = grub_error (GRUB_ERR_BAD_SIGNATURE, N_("error reading digest algorithm: %s"),
                            asn1_strerror (res));
          grub_free (da_path);
          goto cleanup_signed_part;
        }
      ...
      if (grub_strncmp (sha512_oid, algo_oid, algo_oid_size) == 0)
      ...
      else
        {
          err = grub_error (GRUB_ERR_NOT_IMPLEMENTED_YET,
                            N_("only SHA-256 and SHA-512 hashes are supported, found OID %s"),
                            algo_oid);
          grub_free (da_path);
          goto cleanup_signed_part;
        }
  ...
  if (res != ASN1_SUCCESS)
    {
      err = grub_error (GRUB_ERR_BAD_SIGNATURE, N_("error counting number of signers: %s"),
                        asn1_strerror (res));
      goto cleanup_signed_part;
    }
  ...
  if (msg->signerInfos == NULL)
    {
      err = grub_error (GRUB_ERR_OUT_OF_MEMORY,
                        N_("could not allocate space for %d signers"), signer_count);
      goto cleanup_signed_part;
    }
  ...
  for (i = 0; i < signer_count; i++)
    {
      ...
      if (si_da_path == NULL)
        {
          err = grub_error (GRUB_ERR_OUT_OF_MEMORY,
                            N_("could not allocate path for signer %d's digest algorithm parsing path"),
                            i);
      ...
      if (res != ASN1_SUCCESS)
        {
          err = grub_error (GRUB_ERR_BAD_SIGNATURE,
                            N_("error reading signer %d's digest algorithm: %s"), i, asn1_strerror (res));
      ...
      if (grub_strncmp (sha512_oid, algo_oid, algo_oid_size) == 0)
        {
          if (sha512_in_da == false)
            {
              err = grub_error (GRUB_ERR_BAD_SIGNATURE,
                                N_("signer %d claims a SHA-512 signature which was not "
      ...
      else if (grub_strncmp (sha256_oid, algo_oid, algo_oid_size) == 0)
        {
          if (sha256_in_da == false)
            {
              err = grub_error (GRUB_ERR_BAD_SIGNATURE,
                                N_("signer %d claims a SHA-256 signature which was not "
                                   "specified in the outer DigestAlgorithms"), i);
              goto cleanup_signerInfos;
            }
          else
      ...
      if (msg->signerInfos[i].hash == NULL)
        {
          err = grub_error (GRUB_ERR_BAD_SIGNATURE,
                            N_("Hash algorithm for signer %d (OID %s) not loaded"), i, algo_oid);
          goto cleanup_signerInfos;
        }
      ...
      if (si_sig_path == NULL)
        {
          err = grub_error (GRUB_ERR_OUT_OF_MEMORY,
                            N_("could not allocate path for signer %d's signature parsing path"), i);
          goto cleanup_signerInfos;
        }
      ...
      if (gcry_err != GPG_ERR_NO_ERROR)
        {
          err = grub_error (GRUB_ERR_BAD_SIGNATURE,
                            N_("error loading signature %d into MPI structure: %d"),
                            i, gcry_err);
          goto cleanup_signerInfos;
        }
...
parse_pkcs7_signedData (const void *sigbuf, grub_size_t data_size, struct pkcs7_signedData *msg)
{
  ...
  if (res != ASN1_SUCCESS)
    return grub_error (GRUB_ERR_OUT_OF_MEMORY,
                       N_("could not create ASN.1 structure for PKCS#7 data: %s"),
                       asn1_strerror (res));
  ...
  if (res != ASN1_SUCCESS)
    {
      err = grub_error (GRUB_ERR_BAD_SIGNATURE,
                        N_("error decoding PKCS#7 message DER: %s"), asn1_error);
      goto cleanup;
    }
  ...
  if (res != ASN1_SUCCESS)
    {
      err = grub_error (GRUB_ERR_BAD_SIGNATURE, N_("error reading PKCS#7 content type: %s"),
                        asn1_strerror (res));
      goto cleanup;
    }
  ...
  if (grub_strncmp (signedData_oid, content_oid, content_oid_size) != 0)
    {
      err = grub_error (GRUB_ERR_BAD_SIGNATURE,
                        N_("unexpected content type in PKCS#7 message: OID %s"), content_oid);
      goto cleanup;
    }
