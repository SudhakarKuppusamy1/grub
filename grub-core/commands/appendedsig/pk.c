                    ...
                    const char *algo)
{
  ...
  if (rc != GPG_ERR_NO_ERROR)
    {
      ret = grub_error (ret, "building sexp for hash data failed: %s", gpg_strerror (rc));
      goto exit;
    }
  ...
  if (rc != GPG_ERR_NO_ERROR)
    ret = grub_error (ret, "RSA verify failed: %s", gpg_strerror (rc));
  else
    ret = GRUB_ERR_NONE;
                    ...
                    const char *algo)
{
  ...
  rc = _gcry_sexp_build (&s_sig, &errof, "(sig-val (%s (s %b)))", algo, sig, sig_len);
  if (rc != GPG_ERR_NO_ERROR)
    {
      ret = grub_error (ret, "building sexp for signature failed: %s", gpg_strerror (rc));
      goto exit;
