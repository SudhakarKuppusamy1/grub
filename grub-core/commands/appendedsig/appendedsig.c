...
static bool
is_hash_present_in_dbx (grub_uint8_t *const hash, const grub_size_t hash_size)
{
  grub_uint32_t i;
  ...
  for (i = 0; i < dbx.hash_entries; i++)
    {
      if (hash_size == dbx.hash_size[i] &&
        ...
        {
          grub_dprintf ("appendedsig", "the hash (%02x%02x%02x%02x) is ignored"
          ...
          struct grub_database *database, const bool is_db)
{
  ...
  if (is_hash_present_in_dbx (data, data_size) == true)
    {
      grub_dprintf ("appendedsig", "cannot add a hash, as it is present in the %s list",
                    ((is_db == true) ? "db" : "dbx"));
  ...
  if (is_db == true)
    {
      if (is_hash_present_in_db (data, data_size) == true)
        return grub_error (GRUB_ERR_EXISTS,
                           "cannot a add hash, as it is present in the db list");
    }

  hashes = grub_realloc (database->hashes, sizeof (grub_uint8_t *) * (database->hash_entries + 1));
  if (hashes == NULL)
    return grub_error (GRUB_ERR_OUT_OF_MEMORY, "out of memory");

  hash_size = grub_realloc (database->hash_size, sizeof (grub_size_t) * (database->hash_entries + 1));
  if (hash_size == NULL)
    {
      /*
  ...
  grub_dprintf ("appendedsig",
                ...
                data[0], data[1], data[2], data[3], data_size, ((is_db == true) ? "db" : "dbx"));
...
is_cert_hash_present_in_dbx (const grub_uint8_t *data, const grub_size_t data_size)
{
  ...
  for (i = 0; i < dbx.hash_entries; i++)
    {
      rc = generate_cert_hash (dbx.hash_size[i], data, data_size, cert_hash, &cert_hash_size);
      if (rc != GRUB_ERR_NONE)
        continue;
...
static bool
is_cert_fingerprint_match (const grub_uint8_t *hash_data, const grub_size_t hash_data_size,
                           const struct x509_certificate *cert)
{
  if (grub_memcmp (cert->fingerprint[0], hash_data, hash_data_size) == 0
      || grub_memcmp (cert->fingerprint[1], hash_data, hash_data_size) == 0
      || grub_memcmp (cert->fingerprint[2], hash_data, hash_data_size) == 0)
    return true;
...
remove_hash_from_db (const grub_uint8_t *hash_data, const grub_size_t hash_data_size,
                     const bool is_binary_hash)
{
  grub_uint32_t i;
  struct x509_certificate *cert;

  if (is_binary_hash == true)
    {
      for (i = 0; i < db.hash_entries; i++)
        {
          if (grub_memcmp (db.hashes[i], hash_data, hash_data_size) == 0)
  ...
  else
    {
      for (cert = db.certs; cert != NULL; cert = cert->next)
        {
          if (is_cert_fingerprint_match (hash_data, hash_data_size, cert) == true)
            {
              _remove_cert_from_db (cert);
              break;
            }
...
verify_binary_hash (const grub_uint8_t *data, const grub_size_t data_size)
{
  ...
  grub_uint8_t hash[MAX_HASH_SIZE] = { 0 };
  ...
  for (i = 0; i < dbx.hash_entries; i++)
    {
      ...
      rc = get_binary_hash (dbx.hash_size[i], data, data_size, hash, &hash_size);
      if (rc != GRUB_ERR_NONE)
        continue;
  ...
  for (i = 0; i < db.hash_entries; i++)
    {
      ...
      rc = get_binary_hash (db.hash_size[i], data, data_size, hash, &hash_size);
      if (rc != GRUB_ERR_NONE)
        continue;

      if (hash_size == db.hash_size[i] &&
 ...
 * does not persist across reboots. If static key management is enabled, the
 * append_rm_dbx_cert command is only available in the GRUB console. Else the
 * append_add_dbx_cert command is available in the GRUB console.
 */
static grub_err_t
grub_cmd_dbx_cert (grub_command_t cmd __attribute__ ((unused)), int argc, char **args)
{
  ...
  if (grub_pks_keystore.use_keystore == true)
    {
      err = add_certificate (cert_data, cert_data_size, &dbx, false);
      if (err != GRUB_ERR_NONE)
        return err;
    }
          ...
          if (db.hashes[i] != NULL)
            {
              ...
              dump_data_to_hex (db.hashes[i], db.hash_size[i]);
            }
        }
    }
      ...
      if (dbx.hashes[i] != NULL)
        {
          ...
          grub_printf ("    Hash: sha%" PRIuGRUB_SIZE "\n         ", dbx.hash_size[i] * 8);
          dump_data_to_hex (dbx.hashes[i], dbx.hash_size[i]);
        }
    }
...
grub_cmd_db_hash (grub_command_t cmd __attribute__((unused)), int argc, char**args)
{
  ...
  rc = add_hash (hash_data, hash_data_size, &db, true);
  grub_free (hash_data);
                   ...
                   char **args __attribute__ ((unused)))
{
  ...
  rc = add_hash (hash_data, hash_data_size, &dbx, false);
  grub_free (hash_data);
  ...
  if ((*val == '1') || (*val == 'd'))
    {
       ...
       * load the PKS.
       */
      if (grub_pks_keystore.pks_supported == true && grub_pks_keystore.use_keystore == false)
        build_pks_keystore ();
