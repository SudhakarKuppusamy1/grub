  ...
  if (sb_database->is_db == true)
    {
      if (check_hash_presence (data, data_size, &dbx) == true)
        return grub_error (GRUB_ERR_ACCESS_DENIED,
                           "cannot a add hash, as it is present in the dbx list");
    }
      ...
      if (is_cert_match (cert, cert_in) == true)
        {
          grub_dprintf ("appendedsig",
                        ...
                        cert_in->subject, ((sb_database->is_db == true) ? "db" : "dbx"));
          return true;
        }
    }
...
grub_cmd_db_hash (grub_command_t cmd __attribute__((unused)), int argc, char**args)
{
  ...
  /* Only add the hash to the db list if dynamic key management is enabled. */
  if (append_key_mgmt == true)
                   ...
                   char **args __attribute__ ((unused)))
{
  ...
  /* Add the hash to the dbx list if dynamic key management is enabled. */
  if (append_key_mgmt == true)
    {
...
static grub_command_t cmd_verify, cmd_list_db, cmd_db_cert, cmd_db_hash,
                      cmd_list_dbx, cmd_dbx_cert, cmd_add_dbx_cert;
...
GRUB_MOD_INIT (appendedsig)
{
  ...
  cmd_add_dbx_cert = grub_register_command ("append_add_dbx_cert", grub_cmd_dbx_cert, N_("X509_CERTIFICATE"),
...
GRUB_MOD_FINI (appendedsig)
{
  ...
  grub_unregister_command (cmd_add_dbx_cert);
