      ...
      if (is_cert_match (curr_cert, cert) == true)
        {
          ...
          grub_dprintf ("appendedsig",
                        "removed distrused certificate with CN: %s from the db list\n",
                        curr_cert->subject);
          curr_cert->next = NULL;
 ...
 * by separating each byte with ":". So, we introduced a new method dump_ascii_to_hex
 * to display it.
 * Example: Sha256 hash data
  ...
  if (is_cert_present_in_dbx (cert) == false)
    {
      if (is_cert_present_in_db (cert) == true)
        err = grub_error (GRUB_ERR_STILL_REFERENCED,
                          "could not add trusted certificate, as it is present in the db list");
    }
  else
    err = grub_error (GRUB_ERR_STILL_REFERENCED,
