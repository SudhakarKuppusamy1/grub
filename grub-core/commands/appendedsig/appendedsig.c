...
add_hash (grub_uint8_t *const data, const grub_size_t data_size, const bool is_db)
{
  ...
  /* Only accept SHA256, SHA384 and SHA512 hash */
  ...
  new_hash->hash = grub_zalloc (data_size);
  ...
  if (is_db == true)
    {
      new_hash->next = (db.hashes != NULL ? db.hashes : NULL);
      db.hashes = new_hash;
      db.no_of_hashes++;
    }
  else
    {
      new_hash->next = (dbx.hashes != NULL ? dbx.hashes : NULL);
      dbx.hashes = new_hash;
      dbx.no_of_hashes++;
    }
...
is_cert_hash_present_in_dbx (const grub_uint8_t *data, const grub_size_t data_size)
{
  ...
  for (curr_hash = dbx.hashes; curr_hash != NULL; curr_hash = curr_hash->next)
    {
      rc = generate_cert_hash (curr_hash->hash_size, data, data_size, cert_hash, &cert_hash_size);
      if (rc != GRUB_ERR_NONE)
        continue;

      if (cert_hash_size == curr_hash->hash_size &&
          grub_memcmp (curr_hash->hash, cert_hash, cert_hash_size) == 0)
        return true;
    }
                     ...
                     const bool bin_hash)
{
  grub_uint32_t i = 1;
  grub_x509_cert_t *cert;
  grub_hash_t *curr_hash, *prev_hash;

  if (bin_hash == true)
    {
      for (curr_hash = db.hashes; curr_hash != NULL; curr_hash = curr_hash->next, i++)
        {
          if (hash_data_size == curr_hash->hash_size &&
              grub_memcmp (curr_hash->hash, hash_data, hash_data_size) == 0)
            {
              if (i == 1) /* Match with first hash in the db list. */
                db.hashes = curr_hash->next;
              else
...
grub_verify_appended_signature (const grub_uint8_t *buf, grub_size_t bufsize)
{
  ...
  if (db.no_of_certs > 0 && (err == GRUB_ERR_EOF || append_key_mgmt == false))
    {
  ...
  if (err == GRUB_ERR_EOF)
    return grub_error (err, "there is no valid key/hash to verify the signature/file");
