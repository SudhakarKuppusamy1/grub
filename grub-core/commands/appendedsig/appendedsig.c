...
/*
 * The static db list is used to validate appended signatures when
 * secure boot is enabled with static key management.
 */
struct grub_database static_db = {.certs = NULL, .cert_entries = 0, .signatures = NULL,
...
/*
 * It changes the keys from dynamic to static or vice versa when the user
 * changes the appended signature key management from static to dynamic or
 * vice versa.
 *
 * Also, Note that it is allowed to the user when GRUB is not locked down,
 * and this change does not persist across reboots.
 */
static void
load_keys (bool use_keystore_prev_state)
{
  /*
   * If static key management is enabled or dynamic key management is used
   * with the static db as default keys, then the static db should not be
   * freed by free_db_list.
   */
  if (grub_pks_keystore.use_static_keys == true || use_keystore_prev_state == false)
    {
      db.certs = NULL;
      db.cert_entries = 0;
    }
...
{
  char *ret;
  bool use_keystore_prev_state = false;
...
/* Free static db list memory */
static void
free_static_db_list (void)
{
  struct x509_certificate *cert;

  while (static_db.certs != NULL)
    {
      cert = static_db.certs;
      static_db.certs = static_db.certs->next;
      certificate_release (cert);
      grub_free (cert);
    }

  grub_memset (&static_db, 0, sizeof (struct grub_database));
}
...
build_static_db_list (void)
{
  ...
  FOR_MODULES (header)
    {
      ...
      err = add_certificate (cert_data, cert_data_size, &static_db, true);
      grub_free (cert_data);
      if (err == GRUB_ERR_OUT_OF_MEMORY)
        break;
    }
   ...
   * use static db for appended signatures verification.
   */
  if (grub_pks_use_keystore == false || grub_pks_keystore.use_static_keys == true)
    {
      db.certs = static_db.certs;
      db.cert_entries = static_db.cert_entries;
...
build_pks_keystore (void)
{
  ...
  err = grub_pks_load_keystore ();
  if (err != GRUB_ERR_NONE)
    {
      grub_printf ("warning: loading of Platform KeyStore failed\n");
      return;
    }
...
GRUB_MOD_INIT (appendedsig)
{
  ...
  /* Extract trusted keys from ELF Note and store them in the static db list. */
  build_static_db_list ();
  /*
