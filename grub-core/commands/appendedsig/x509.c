...
grub_parse_rsa_pubkey (grub_uint8_t *der, int dersize, struct x509_certificate *certificate)
{
  ...
  if (result != ASN1_SUCCESS)
    {
      err = grub_error (GRUB_ERR_BAD_FILE_TYPE,
                        N_("cannot decode certificate public key DER: %s"), asn1_error);
      goto cleanup;
    }
  ...
  if (gcry_err != GPG_ERR_NO_ERROR)
    {
      err = grub_error (GRUB_ERR_BAD_FILE_TYPE,
                        N_("error loading RSA modulus into MPI structure: %d"), gcry_err);
      goto cleanup_e_data;
    }
  ...
  if (gcry_err != GPG_ERR_NO_ERROR)
    {
      err = grub_error (GRUB_ERR_BAD_FILE_TYPE,
                        N_("error loading RSA exponent into MPI structure: %d"), gcry_err);
      goto cleanup_m_mpi;
    }
...
grub_x509_read_subject_public_key (asn1_node asn, struct x509_certificate *results)
{
  ...
  if (result != ASN1_SUCCESS)
    return grub_error (GRUB_ERR_BAD_FILE_TYPE, N_("error reading x509 public key algorithm: %s"),
                       asn1_strerror (result));

  if (grub_strncmp (algo_oid, rsaEncryption_oid, sizeof (rsaEncryption_oid)) != 0)
    return grub_error (GRUB_ERR_NOT_IMPLEMENTED_YET,
                       N_("unsupported x509 public key algorithm: %s"), algo_oid);
  ...
  if (result != ASN1_SUCCESS)
    return grub_error (GRUB_ERR_BAD_FILE_TYPE, N_("error reading x509 public key parameters: %s"),
                       asn1_strerror (result));
  ...
  if (result != ASN1_MEM_ERROR)
    return grub_error (GRUB_ERR_BAD_FILE_TYPE, N_("error reading size of x509 public key: %s"),
                       asn1_strerror (result));
  if (key_type != ASN1_ETYPE_BIT_STRING)
    return grub_error (GRUB_ERR_BAD_FILE_TYPE, N_("unexpected ASN.1 type when reading x509 public key: %x"),
                       key_type);
...
decode_string (char *der, int der_size, char **string, grub_size_t *string_size)
{
  ...
  if (result != ASN1_SUCCESS)
    return grub_error (GRUB_ERR_OUT_OF_MEMORY,
                       N_("could not create ASN.1 structure for certificate: %s"),
                       asn1_strerror (result));
  ...
  if (result != ASN1_SUCCESS)
    {
      err = grub_error (GRUB_ERR_BAD_FILE_TYPE,
                        N_("could not parse DER for DirectoryString: %s"), asn1_error);
      goto cleanup;
    }
  ...
  if (grub_strncmp ("utf8String", choice, choice_size) == 0)
    {
      ...
      if (result != ASN1_MEM_ERROR)
        {
          err = grub_error (GRUB_ERR_BAD_FILE_TYPE, N_("error reading size of UTF-8 string: %s"),
                            asn1_strerror (result));
          goto cleanup_choice;
        }
    }
  ...
  else
    {
      err = grub_error (GRUB_ERR_NOT_IMPLEMENTED_YET,
                        N_("only UTF-8 and printable DirectoryStrings are supported, got %s"),
                        choice);
      goto cleanup_choice;
    }
  ...
  *string = grub_malloc (tmp_size);
  ...
  if (result != ASN1_SUCCESS)
    {
      err = grub_error (GRUB_ERR_BAD_FILE_TYPE, N_("error reading out %s in DirectoryString: %s"),
...
check_version (asn1_node certificate)
{
  ...
  if (rc != ASN1_SUCCESS || len != 1)
    return grub_error (GRUB_ERR_BAD_FILE_TYPE, N_("error reading certificate version"));

  if (version != 0x02)
    return grub_error (GRUB_ERR_BAD_FILE_TYPE,
                       N_("invalid x509 certificate version, expected v3 (0x02), got 0x%02x"),
                       version);
...
read_name (asn1_node asn, const char *name_path, char **name, grub_size_t *name_size)
{
  ...
  *name = NULL;
  ...
  if (top_path == NULL)
    return grub_error (GRUB_ERR_OUT_OF_MEMORY,
                       N_("could not allocate memory for %s name parsing path"), name_path);

  result = asn1_number_of_elements (asn, top_path, &seq_components);
  if (result != ASN1_SUCCESS)
    {
      err = grub_error (GRUB_ERR_BAD_FILE_TYPE, N_("error counting name components: %s"),
                        asn1_strerror (result));
      goto cleanup;
    }
  ...
  for (i = 1; i <= seq_components; i++)
    {
      ...
      if (set_path == NULL)
        {
          err = grub_error (GRUB_ERR_OUT_OF_MEMORY,
                            N_("could not allocate memory for %s name set parsing path"),
                            name_path);
          goto cleanup_set;
        }
      ...
      if (result != ASN1_SUCCESS)
        {
          err = grub_error (GRUB_ERR_BAD_FILE_TYPE,
                            N_("error counting name sub-components components (element %d): %s"),
      ...
      for (j = 1; j <= set_components; j++)
        {
          ...
          if (type_path == NULL)
            {
              err = grub_error (GRUB_ERR_OUT_OF_MEMORY,
                                N_("could not allocate memory for %s name component type path"),
                                name_path);
              goto cleanup_set;
            }
          ...
          if (result != ASN1_SUCCESS)
            {
              err = grub_error (GRUB_ERR_BAD_FILE_TYPE, N_("error reading %s name component type: %s"),
          ...
          if (val_path == NULL)
            {
              err = grub_error (GRUB_ERR_OUT_OF_MEMORY,
                                N_("could not allocate memory for %s name component value path"),
                                name_path);
              goto cleanup_type;
            }
...
verify_key_usage (grub_uint8_t *value, int value_size)
{
  ...
  if (result != ASN1_SUCCESS)
    {
      err = grub_error (GRUB_ERR_BAD_FILE_TYPE, N_("error reading Key Usage value: %s"),
                        asn1_strerror (result));
      goto cleanup;
    }
  ...
  if (!(usage & digitalSignatureUsage))
    {
      err = grub_error (GRUB_ERR_BAD_FILE_TYPE,
                        N_("key usage (0x%x) missing Digital Signature usage"), usage);
      goto cleanup;
    }
...
verify_basic_constraints (grub_uint8_t *value, int value_size)
{
  ...
  if (result != ASN1_SUCCESS)
    {
      err = grub_error (GRUB_ERR_BAD_FILE_TYPE,
                        N_("error parsing DER for Basic Constraints: %s"), asn1_error);
      goto cleanup;
    }
  ...
  if (result == ASN1_ELEMENT_NOT_FOUND)
    ...
    {
      err = grub_error (GRUB_ERR_BAD_FILE_TYPE, N_("error reading Basic Constraints cA value: %s"),
                        asn1_strerror (result));
      goto cleanup;
    }
  ...
  if (grub_strncmp ("FALSE", cA, cA_size) != 0)
    {
      err = grub_error (GRUB_ERR_BAD_FILE_TYPE, N_("unexpected CA value: %s"), cA);
      goto cleanup;
    }
...
verify_extended_key_usage (grub_uint8_t *value, int value_size)
{
  ...
  if (result != ASN1_SUCCESS)
    {
      err = grub_error (GRUB_ERR_BAD_FILE_TYPE,
                        N_("error parsing DER for Extended Key Usage: %s"), asn1_error);
      goto cleanup;
    }
  ...
  if (result != ASN1_SUCCESS)
    {
      err = grub_error (GRUB_ERR_BAD_FILE_TYPE, N_("error counting number of Extended Key Usages: %s"),
                        asn1_strerror (result));
      goto cleanup;
    }
  ...
  for (i = 1; i < count + 1; i++)
    {
      ...
      if (result != ASN1_SUCCESS)
        {
          err = grub_error (GRUB_ERR_BAD_FILE_TYPE, N_("error reading Extended Key Usage: %s"),
                            asn1_strerror (result));
          goto cleanup;
        }
...
verify_extensions (asn1_node cert)
{
  ...
  if (result != ASN1_SUCCESS)
    return grub_error (GRUB_ERR_BAD_FILE_TYPE, N_("error counting number of extensions: %s"),
                       asn1_strerror (result));

  if (num_extensions < 2)
    return grub_error (GRUB_ERR_BAD_FILE_TYPE,
                       N_("insufficient number of extensions for certificate, need at least 2, got %d"),
                       num_extensions);
  ...
  for (ext = 1; ext <= num_extensions; ext++)
    {
      ...
      if (result != ASN1_SUCCESS)
        {
          err = grub_error (GRUB_ERR_BAD_FILE_TYPE, N_("error reading extension OID: %s"),
                            asn1_strerror (result));
      ...
      if (result == ASN1_ELEMENT_NOT_FOUND)
        ...
        {
          err = grub_error (GRUB_ERR_BAD_FILE_TYPE, N_("error reading extension criticality: %s"),
                            asn1_strerror (result));
      ...
      if (grub_strncmp (keyUsage_oid, extnID, extnID_size) == 0)
      ...
      else if (grub_strncmp ("TRUE", critical, critical_size) == 0)
        {
           ...
           */
          err = grub_error (GRUB_ERR_BAD_FILE_TYPE,
                            N_("unhandled critical x509 extension with OID %s"), extnID);
          goto cleanup_value;
        }
  ...
  if (usage_present != 1)
    return grub_error (GRUB_ERR_BAD_FILE_TYPE,
                       N_("unexpected number of Key Usage extensions - expected 1, got %d"),
                       usage_present);

  if (constraints_present != 1)
    return grub_error (GRUB_ERR_BAD_FILE_TYPE,
                       N_("unexpected number of basic constraints extensions - expected 1, got %d"),
                       constraints_present);

  if (extended_usage_present > 1)
    return grub_error (GRUB_ERR_BAD_FILE_TYPE,
                       N_("unexpected number of Extended Key Usage extensions - expected 0 or 1, got %d"),
                       extended_usage_present);
...
parse_x509_certificate (const void *data, grub_size_t data_size, struct x509_certificate *results)
{
  ...
  if (result != ASN1_SUCCESS)
    {
      err = grub_error (GRUB_ERR_BAD_FILE_TYPE,
                        N_("could not parse DER for certificate: %s"), asn1_error);
      goto cleanup;
    }
