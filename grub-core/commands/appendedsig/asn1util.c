...
grub_asn1_allocate_and_read (asn1_node node, const char *name, const char *friendly_name, int *content_size)
{
  ...
  if (result != ASN1_MEM_ERROR)
    {
      grub_error (GRUB_ERR_BAD_FILE_TYPE, N_("reading size of %s did not return expected status: %s"),
  ...
  if (tmpstr == NULL)
    {
      grub_error (GRUB_ERR_OUT_OF_MEMORY, N_("could not allocate memory to store %s"),
                  friendly_name) ;
      return NULL;
    }
  ...
  if (result != ASN1_SUCCESS)
    {
      grub_free (tmpstr);
      grub_error (GRUB_ERR_BAD_FILE_TYPE, N_("error reading %s: %s"), friendly_name,
