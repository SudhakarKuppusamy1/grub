...
appended_signature_test (void)
{
   ...
   * certificate #1.
   */
  if (grub_strncmp (key_mgmt, "static", sizeof(key_mgmt)) == 0)
    {
      cmd_distrust = grub_command_find ("append_rm_dbx_cert");
      if (cmd_distrust == NULL)
        {
          grub_test_assert (0, "can't find command `%s'", "append_rm_dbx_cert");
          return;
        }
  ...
  else
    {
      ...
      /* Now reload certificate #1. */
      err = (cmd_trust->func) (cmd_trust, 1, trust_args);
      grub_test_assert (err != GRUB_ERR_NONE, "unexpectedly loaded certificate 1: %d: %s",
                        grub_errno, grub_errmsg);
      DO_TEST (hi_signed_2nd, 1);
      DO_TEST (hi_signed, 0);
      DO_TEST (hi_double, 1);
      ...
      /* Now reload certificate #2. */
      err = (cmd_trust->func) (cmd_trust, 1, trust_args2);
      grub_test_assert (err != GRUB_ERR_NONE, "unexpectedly loaded certificate 2: %d: %s",
                        grub_errno, grub_errmsg);
      DO_TEST (hi_signed_2nd, 0);
      DO_TEST (hi_signed, 0);
      DO_TEST (hi_double, 0);
